...
def get_sessions(limit: int = 50, offset: int = 0) -> list[dict]:
    ...
    rows = conn.execute(
        "SELECT id, title, created_at, updated_at, model FROM sessions ORDER BY created_at DESC LIMIT ? OFFSET ?",
        (limit, offset),
    ).fetchall()
    conn.close()
...
def get_session_messages(session_id: str, limit: int = 100) -> list[dict]:
    ...
    conn.row_factory = sqlite3.Row
    rows = conn.execute(
        "SELECT id, role, content, created_at, tool_calls FROM messages WHERE session_id = ? ORDER BY created_at LIMIT ?",
        (session_id, limit),
    ).fetchall()
    conn.close()
    msgs = []
...
def search_sessions(q: str, limit: int = 20) -> list[dict]:
    ...
    conn.row_factory = sqlite3.Row
    rows = conn.execute(
        "SELECT id, title, created_at, updated_at FROM sessions WHERE title LIKE ? ORDER BY created_at DESC LIMIT ?",
        (f"%{q}%", limit),
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
